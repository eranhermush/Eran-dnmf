    ...
    for ref_name in refs:
        ...
        for mix_name in mixes_names:
            try:
                ...
                if useW0:
                    deep_nmf, dnmf_train_cost, dnmf_w, out_h = train_unsupervised_new(tensoring(mix_data).T, num_layers, n_iter,
                                                                                  n_components, ref_data=ref_data, l_1=l1,
                                                                                  l_2=l2, w1_opt=w1_opt, dist_mix_i=dist_mix_i)
                else:
                    deep_nmf, dnmf_train_cost, dnmf_w, out_h = train_unsupervised_new(tensoring(mix_data).T, num_layers, n_iter,
                                                                        n_components, l_1=l1, l_2=l2, w1_opt=w1_opt, dist_mix_i=dist_mix_i)
...
if __name__ == '__main__':
    output_folder = "/Users/Eran/Documents/benchmarking-transcriptomics-deconvolution/Figure1/Eran/" \
                    "25.3/2_unsupervised_6.4"
    option_gedit = [False, True, 500]
    option_W0 = [True, False]
    options_w1 = ['algo', '1', 'last', None]
    print("W1 algo")
    for gedit in option_gedit:
        ...
        for w0 in option_W0:
            for w1_opt in options_w1:
                ...
                # output_string = f"Dnmf$Unupervised$W=W1SoftmaxLast${generate_yes_no(gedit)}Gedit${generate_yes_no(w0)}Wo$"
                print(f"{generate_yes_no(gedit)}Gedit${generate_yes_no(w0)} w1 {w1_opt}")
                main_train_on_generated_data(mix_signature_folder, use_gedit=gedit, useW0=w0,
                                             output_prefix=output_string, w1_opt=w1_opt)
