...
"""
from gedit_preprocess import MatrixTools
...
def checkInputs(InputString):
    ...
    if "mixData" in argDict:
        ...
        if "mix" not in argDict:
            return (
                False,
                "Mixture matrix not specified. Please indicate\
    ...
    if mixCheck != True:
        return (
            False,
            "An error was detected with your\
     submitted mixture file:\n"
            + mixCheck,
        )

    if "refData" in argDict:
        Ref = argDict["refData"]
        RefFName = "refData"
    else:
        if "ref" not in argDict:
            return (
                False,
                "reference matrix not specified. Please indicate\
    ...
    if "SigMethod" in argDict:
        ...
        for SigMethod in SigMethodList.split(","):
            if SigMethod not in ["Intensity", "Entropy", "Zscore", "MeanRat", "MeanDiff", "fsRat", "fsDiff", "IntEnt"]:
