...
def returnSigMatrix(inMat, minPerCT, TotalSigs, strModes):
    ...
    """
    validModes = ["Intensity", "Entropy", "Zscore", "MeanRat", "MeanDiff", "fsRat", "fsDiff"]

    trimmed = trimIncomplete(inMat[1:], 0.5)
...
def scoreGene12(StrExpVals, mode):
    ...
    if mode == "ratio":
        return biggest / max(sndBiggest, 0.00000001)
...
def scoreGeneMeanComparison(ExpressionVals, mode):
    ...
    if mode == "Zscore":
        ...
        score = (maxExp - mean) / max(spread, 0.0000001)

    elif mode == "MeanRat":
        score = maxExp / max(mean, 0.000001)
